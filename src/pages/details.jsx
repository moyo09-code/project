...
export default function PropertyDetails() {
  ...
  if (!property) {
    return <p className="p-6 text-center text-gray-700">Property not found.</p>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-4 gap-4">
        <h2 className="text-2xl sm:text-3xl font-bold text-green-900">{property.title}</h2>
        <div className="flex gap-3">
          <button
            onClick={() => toggleLike(property)}
            className={`p-2 rounded-full ${liked ? "bg-red-600 text-white" : "bg-white text-red-600"} hover:scale-110 transition`}
          >
            <FiHeart size={24} />
          </button>
          ...
          <button
            ...
            className={`p-2 rounded-full ${inCart ? "bg-green-700 text-white" : "bg-white text-green-700"} hover:scale-110 transition`}
          >
            <FiPlus size={24} />
          </button>
        </div>
      ...
      <p className="text-gray-500 mt-2 text-sm sm:text-base">{property.location}</p>
      <p className="text-green-800 font-semibold mt-1 text-sm sm:text-base">{property.price}</p>
      ...
      {property.category && (
        <div className="mt-4 text-sm sm:text-base">
          <h3 className="font-bold text-lg sm:text-xl">Category</h3>
